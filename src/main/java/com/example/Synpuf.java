...
public class Synpuf
{
  private static final byte[] FAMILY = Bytes.toBytes("beneficiary-summary");
    ...
    private static final byte[] BENE_HI_CVRAGE_TOT_MONS = Bytes.toBytes("BENE_HI_CVRAGE_TOT_MONS");
    private static final byte[] BENE_SMI_CVRAGE_TOT_MONS = Bytes.toBytes("BENE_SMI_CVRAGE_TOT_MONS");
    private static final byte[] BENE_HMO_CVRAGE_TOT_MONS = Bytes.toBytes("BENE_HMO_CVRAGE_TOT_MONS");
    private static final byte[] PLAN_CVRG_MOS_NUM = Bytes.toBytes("PLAN_CVRG_MOS_NUM");
  ...
  public void processElement(DoFn<String, Mutation>.ProcessContext c) throws Exception {
  	...
  	String line = c.element();
 		...
 		String[] parts = csvParser.parseLine(line);
			...
			put_object.addColumn(FAMILY, BENE_HI_CVRAGE_TOT_MONS, Bytes.toBytes(parts[8]));
			put_object.addColumn(FAMILY, BENE_SMI_CVRAGE_TOT_MONS, Bytes.toBytes(parts[9]));
...
p.apply(TextIO.Read.named("Reading from File").from("gs://synpuf_data/Book1.csv")).apply(ParDo.named("Processing Synpuf data").of(MUTATION_TRANSFORM)).apply(CloudBigtableIO.writeToTable(config));
		...
		//PCollection<String> lines=p.apply(TextIO.Read.from("gs://synpuf-data/Book1.csv"))
